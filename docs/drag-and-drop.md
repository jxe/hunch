...
**Touch-to-row mapping** — the recognizer is attached to the scroll view, not per-row. UIKit reports `recognizer.location(in: scrollView)` in the scroll view's shifted content coordinates, while SwiftUI page geometry uses the page's visible named coordinate space. The bridge subtracts `scrollView.contentOffset` before row lookup, lift anchoring, and drop-slot resolution; otherwise a scrolled page double-counts the vertical offset and starts the drag from a lower row. Drag-source lookup then uses actual frames published by the currently materialized LazyVStack rows in stack-local coordinates. It deliberately does not use the cumulative height cache (which may contain zero or stale heights for off-screen rows) and has no nearest-row fallback, so a press in inter-row spacing does not pick a neighbor. Drop-slot resolution continues to use cumulative geometry because it must cover the whole document during autoscroll.

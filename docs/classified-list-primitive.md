# Classified-list primitive — design
...
Reuse the `:::label` fence machinery in [parseTemplateContainers](../App/Sources/Clamshell/Parser.swift). Add a `:::list` arm. Body is regular markdown (bullets, sub-bullets, divider, more bullets); the fence is the only thing tying them together.
